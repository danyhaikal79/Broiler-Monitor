...
# Paths to the deployed models
# Feeder model: 3-class segmentation (feeder BODY only -- feed level done by CV)
...
def load_feeder_model():
    global _feeder_model
    if _feeder_model is None:
        from ultralytics import YOLO
        if not FEEDER_WEIGHTS.exists():
            raise FileNotFoundError(f"Feeder weights not found: {FEEDER_WEIGHTS}")
        _feeder_model = YOLO(str(FEEDER_WEIGHTS))
    return _feeder_model
...
def run_feeder(image: Image.Image, conf: float = 0.25) -> FeederResult:
    """
    Run the feeder segmentation model (3-class: pan3kg/pan7kg/tube7kg).

    Its only jobs: (1) classify which feeder type, (2) return the feeder body
    polygon + bbox so the CV pipeline can use it as the ROI. Feed level itself
    is measured by the classical-CV pipeline (cv_feeder.py), NOT here.
    """
    model = load_feeder_model()
