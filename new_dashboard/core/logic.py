...
# ----------------------------------------------------------------------------
# Heat-stress adjustment factor (derived from Purswell et al. 2012, Fig.2 / Tab.2)
#
# Purswell measured feed intake at 9 THI points (14.8 .. 26.6 °C). The data
# shows: ~no decline below ~21°C THI, then linear decline of ~5%/°C above 21.
# We approximate with a piecewise-linear function clamped to [0.65, 1.0].
# ----------------------------------------------------------------------------
def thi_stress_factor(thi_c: float) -> float:
    """1.0 = no stress (full intake). Lower values = chickens eat less."""
    if thi_c <= 21.0:
        return 1.0
    if thi_c >= 27.0:
        return 0.70
    return 1.0 - 0.05 * (thi_c - 21.0)   # linear 1.0 -> 0.70 between 21..27
